...
# jsonify = ver melhor as respostas
# request = fazer operaçoes com o flask
...
def token_obrigatorio(f):
    @wraps(f)
    def decorated(*args,**kwargs): #*posicionais, **nomeados
        token = None
        # Verificar se um token foi enviado no headers
        if 'x-access-token' in request.headers: # armazenando/acessando informações extras que contem o token
        ...
        try:
            ...
            # extrair as informaçoes que estão dentro desse token e verificar se essas informações estão cadastradas no banco de dados
...
@app.route('/login')
def login():
    auth = request.authorization # para extrair as informações que foram passadas de autenticação para API (usuário e senha)
    if not auth or not auth.username or not auth.password:
        return make_response('Login inválido', 401, {'WWW-Authenticate':'Basic realm="Login Obrigatório"'}) # tela/solicitação de login
    
    usuario = Autor.query.filter_by(nome=auth.username).first() # verificar se o usuario existe no banco de dados
    if not usuario:
        return make_response('Login inválido', 401, {'WWW-Authenticate':'Basic realm="Login Obrigatório"'})

    if auth.password == usuario.senha:
        token = jwt.encode({'id_autor': usuario.id_autor, 'exp': datetime.utcnow(
        ) + timedelta(minutes=30)}, app.config['SECRET_KEY'], algorithm='HS256') # retornar token
...
# Obter postagem por id - GET https://localhost:7777/postagem/1
...
# Criar uma nova postagem - POST https://localhost:7777/postagem
...
# Alterar uma postagem existente - PUT https://localhost:7777/postagem/1
...
# Excluir uma postagem - DELETE - https://localhost:7777/postagem/1
...
def obter_autores(autor):
    autores = Autor.query.all() # estrair todos os dados da tabela(class)
    # retornar um lista com todos os autores
    lista_de_autores = []
...
def excluir_autor(id_autor):
    # Checar se o autor existe no banco de dados
